...
export class AuthService {
  ...
  async signup(data: SignupGQLDTO): Promise<string> {
    const { email } = data;
  ...
  async resendConfirmEmail({email} : ResendConfirmEmailGQLDTO) : Promise<string>{
  ...
  async confirmEmail(data : ConfirmOTPGQLDTO) : Promise<string>{
  ...
  async login(data: LoginGQLDTO , issure : string) : Promise<IGenerateToken> {
  ...
  async forgetPassword(data : ForgetPasswordGQLDTO) : Promise<string>{
    ...
    await isKeyExpired({email : email || phone as string , type : RedisTypeEnum.FORGETPASSWORD})
  ...
  async resetPassword({email , password} : ResetPasswordGQLDTO) : Promise<string>{
  const user = await this.UserRepository.findOne({filter: {
      email,
      confirmedAt: { $exists: true },
      provider: ProviderEnum.SYSTEM,
